...
def fetchmetainfo(metadata, GClinkfh, outfh):
    namedict = dict()
    ...
    with open(metadata, 'r') as fh0:
        for l0 in fh0:
            if not l0.startswith('Study'):
                ...
                simstudyid = studyid.replace("PE","")
                namedict[simstudyid] = l0.strip()

    with open(outfh, 'w') as fo:
        fo.write('GCcode\tStudyID\tCase.Ctrl\tCase.Control\tMAC\tGAD.weeks\tGA.NIPT.Weeks\tFF\tNIPT.result\tStudy.Code.NIPT\tbisulfite.conv.NIPT1\t%regions.with.less.than.50.NIPT1\tBMI\tProteinuria\tThrombocytopenia\tLiver.transaminase\tSevere.PET\tGestational.DM\tSex.newborn\tSex.newborn2\n')
        with open(GClinkfh, 'r') as fh1:
            for l1 in fh1:
                (GCcode, tempname) = l1.strip().split('\t')
                matchid = tempname[0:3]
                if matchid in namedict:
                    pfline = f'{GCcode}\t{namedict[matchid]}\n'
                    fo.write(pfline)


def mergecovinfo(seqcovfh, outmergefh):
    pedict = dict()
...
if __name__ == '__main__':
    ...
    #mergecovinfo(sys.argv[1], sys.argv[2])
