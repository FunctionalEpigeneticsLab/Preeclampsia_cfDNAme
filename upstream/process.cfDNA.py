    ...
    for sampleid in SampleIDs:
        ...
        with open(samplescript, 'w+') as fo:
            ...
            for fastqfile in os.listdir(fastqdir):
                if fastqfile.startswith(sampleid) and (fastqfile.endswith('R1.fastq.gz') or fastqfile.endswith('R2.fastq.gz')):
            ...
            for fastqfile in os.listdir(fastqdir):
                if fastqfile.startswith(sampleid) and fastqfile.endswith('R1.fastq.gz'):
                    ...
                    if (DNAtrimoption == "default"):
                        ...
                        addparam = '%s %s' % (baseparam, DNAtrimoption)
                    #trim_task = '%s --length 36 --clip_R1 9 --clip_R2 9 --three_prime_clip_R1 2 -o %s --paired %s --fastqc\n\n' % (trimtool, trim_workdir, readpairs)
...
def write_sub_script_DNAalign(DNASampleIDs, fastqdir, scriptdir, workdir, logdir):
    ...
    for sampleid in DNASampleIDs:
        ...
        with open(samplescript, 'w') as fo:
            ...
            for fastqfile in os.listdir(fastqdir):
                if fastqfile.startswith(sampleid) and fastqfile.endswith('R1.fastq.gz'):
                #if fastqfile.startswith(sampleid) and fastqfile.endswith('R1_val_1.fq.gz'):
                    curfastqR1 = '%s/%s' % (fastqdir, fastqfile)
                    lanefastqR1.append(curfastqR1)

            FQR1 = ','.join(lanefastqR1)
            trimFQR1 = FQR1.replace(fastqdir, trim_workdir).replace("R1.fastq.gz","R1_val_1.fq.gz")
            ...
            #alignbams = FQR1.replace(fastqdir, sample_sub_workdir).replace(".fastq.gz","_bismark_bt2_pe.bam").replace(",", " ")
