...
plotvenn <- function(sumfh,outfigprefix) {
    ...
    vlists3 <- list(FreshTissue_hypo,cfDNA_atDiagnosis_hypo,cfDNA_1stTrimester_hypo)
    outfig3 <- paste0(outfigprefix,".hypo.3way_fresh_cfDNA2_cfDNA1.overlap.venn.pdf")
    pdf(outfig3,height=7,width=7)
    p3 <- display_venn(
            vlists3,
            category.names = c("Placenta","cfDNA at diagnosis","cfDNA 1st trimester"),
            # Circles
            lwd = 2,
            lty = 'blank',
            fill = c("#a6cee3","#bdbdbd","#b2df8a"),
    ...
    vlists4 <- list(FreshTissue_hyper,cfDNA_atDiagnosis_hyper,cfDNA_1stTrimester_hyper)
    outfig4 <- paste0(outfigprefix,".hyper.3way_fresh_cfDNA2_cfDNA1.overlap.venn.pdf")
    pdf(outfig4,height=7,width=7)
    p4 <- display_venn(
            vlists4,
            category.names = c("Placenta","cfDNA at diagnosis","cfDNA 1st trimester"),
            # Circles
...
plotupset <- function(sumfh,outfigprefix) {
    ...
    MAY6_hypo <- fh[fh$Material=="cfDNA_1stTrimester_valid" & fh$Type=="Hypomethylated",]$Index
    MAY6_hyper <- fh[fh$Material=="cfDNA_1stTrimester_valid" & fh$Type=="Hypermethylated",]$Index
    ...
    vlists6 <- list(`cfDNA at diagnosis hypo`=MAY4_hypo,`cfDNA 1st trimester hypo`=MAY5_hypo,`cfDNA at diagnosis hyper`=MAY4_hyper,`cfDNA 1st trimester hyper`=MAY5_hyper)
    outfig6 <- paste0(outfigprefix,".2way.hypohyper.cfDNA2.cfDNA1.overlap.upsetplot.pdf")
    pdf(outfig6,height=5.6,9.8)
    p6 <- upset(fromList(vlists6),sets=c("cfDNA at diagnosis hypo","cfDNA 1st trimester hypo","cfDNA at diagnosis hyper","cfDNA 1st trimester hyper"),keep.order=TRUE,point.size=2,line.size=0.8,mainbar.y.label="Overlap regions",sets.x.label="Regions per material",text.scale = c(1.5, 1.5, 1.5, 1.3, 1.5, 1),main.bar.color="steelblue",empty.intersections="on")
    print(p6,newpage=FALSE)
    dev.off()

    vlists7 <- list(`cfDNA 1stT discovery hypo`=MAY5_hypo,`cfDNA 1stT validation hypo`=MAY6_hypo,`cfDNA 1stT discovery hyper`=MAY5_hyper,`cfDNA 1stT validation hyper`=MAY6_hyper)
    outfig7 <- paste0(outfigprefix,".2way.hypohyper.cfDNA1d.cfDNA1v.overlap.upsetplot.pdf")
    pdf(outfig7,height=5.6,9.8)
    p7 <- upset(fromList(vlists7),sets=c("cfDNA 1stT discovery hypo","cfDNA 1stT validation hypo","cfDNA 1stT discovery hyper","cfDNA 1stT validation hyper"),keep.order=TRUE,point.size=2,line.size=0.8,mainbar.y.label="Overlap regions",sets.x.label="Regions per material",text.scale = c(1.5, 1.5, 1.5, 1.3, 1.5, 1),main.bar.color="steelblue",empty.intersections="on")
