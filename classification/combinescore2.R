...
# Combine eRPM/FMF scores with PEscores
...
TrainingCombineFMF <- function(infh,FMFscore,trainrocfig, trainoutfh, traincombrocfig) {
    ...
    fhall <- fhall[!is.na(fhall$FMFSCORE34),]
    print(head(fhall))
    fh <- subset(fhall, Set=="Discovery")
    pescoreperfm <- roc(response=fh$Phenotype,predictor=fh$PEscore,ci=TRUE,levels=c("Ctrl","Case"),direction="<")
    fmfperfm <- roc(response=fh$Phenotype,predictor=fh[,c(FMFscore)],ci=TRUE,levels=c("Ctrl","Case"),direction=">")
    print("ok")
    #fmfperfm <- roc(response=fh$Phenotype,predictor=fh$FMFscore,ci=TRUE,levels=c("Ctrl","Case"),direction=">")
    ...
    dev.off()
    predt <- data.frame()
    fh$Pheno <- ifelse(fh$Phenotype=="Ctrl",0,1)
    fh$log10FMFSCORE <- log10(fh[,c(FMFscore)])

    trainallglm <- glm(Pheno ~ PEscore+log10FMFSCORE, family=binomial(link='logit'), data=fh)

    n_train <- nrow(fh)
    for (i in 1:n_train) {
        traindt <- fh[-i,]
	testvars <- as.data.frame(fh[i,c("PEscore","log10FMFSCORE")])
	trainglm <- glm(Pheno ~ PEscore+log10FMFSCORE, family=binomial(link='logit'), data=traindt)
    ...
    outdt <- cbind(fh[,c("SubjectID","Phenotype","PEscore","PredClass","FMFRISK34","Center")],predt)
    write.table(outdt, trainoutfh, row.names=FALSE, quote=FALSE, sep="\t")

    combineperfm <- roc(response=outdt$Phenotype,predictor=outdt$CombinedScore,ci=TRUE,levels=c("Ctrl","Case"),direction="<")
...
ValidCombineFMF <- function(infh,FMFscore,validoutfh, validrocfig, valid1rocfig, valid2rocfig, classoutfig) {
    ...
    fh <- fh[!is.na(fh$FMFSCORE34),]
    fh$Pheno <- ifelse(fh$Phenotype=="Ctrl",0,1)
    fh$log10FMFSCORE <- log10(fh[,c(FMFscore)])
    traindt <- subset(fh, Set=="Discovery")
    testdt <- subset(fh, Set=="Validation")
    ...
    testvars <- testdt[,c("PEscore","log10FMFSCORE")]

    testpred <- predict(trainglm,newdata=testvars,type="response")
    testpredclass <- ifelse(testpred > 0.5,"Case","Ctrl")
    predt <- cbind(testdt[,c("SubjectID","Phenotype","PEscore","PredClass",FMFscore,"FMFRISK34","log10FMFSCORE","Center")],testpred,testpredclass)
    write.table(predt,validoutfh,row.names=FALSE,quote=FALSE,sep="\t")

    validpespf <- roc(response=predt$Phenotype,predictor=predt$PEscore,ci=TRUE,levels=c("Ctrl","Case"),direction="<")
    validlfmfpf <- roc(response=predt$Phenotype,predictor=predt$log10FMFSCORE,ci=TRUE,levels=c("Ctrl","Case"),direction=">")
    validpeadlfmfpf <- roc(response=predt$Phenotype,predictor=predt$testpred,ci=TRUE,levels=c("Ctrl","Case"),direction="<")
    ...
    ZOLvalidpespf <- roc(response=ZOLpredt$Phenotype,predictor=ZOLpredt$PEscore,ci=TRUE,levels=c("Ctrl","Case"),direction="<")
    ZOLvalidlfmfpf <- roc(response=ZOLpredt$Phenotype,predictor=ZOLpredt$log10FMFSCORE,ci=TRUE,levels=c("Ctrl","Case"),direction=">")
    ZOLvalidpeadlfmfpf <- roc(response=ZOLpredt$Phenotype,predictor=ZOLpredt$testpred,ci=TRUE,levels=c("Ctrl","Case"),direction="<")
    ...
    SJBvalidpespf <- roc(response=SJBpredt$Phenotype,predictor=SJBpredt$PEscore,ci=TRUE,levels=c("Ctrl","Case"),direction="<")
    SJBvalidlfmfpf <- roc(response=SJBpredt$Phenotype,predictor=SJBpredt$log10FMFSCORE,ci=TRUE,levels=c("Ctrl","Case"),direction=">")
    SJBvalidpeadlfmfpf <- roc(response=SJBpredt$Phenotype,predictor=SJBpredt$testpred,ci=TRUE,levels=c("Ctrl","Case"),direction="<")
    ...
    classdf <- predt[,c("SubjectID","Phenotype","PredClass","FMFRISK34","testpredclass")]
...
DiscoveryCombineSeqFF_FMF <- function(infh,trainrocfig, trainoutfh, traincombrocfig) {
    fhall <- fread(infh, header=TRUE, sep="\t", data.table=FALSE)
    fh <- subset(fhall, Set=="Discovery")
    pescoreperfm <- roc(response=fh$Phenotype,predictor=fh$PEscore,ci=TRUE,levels=c("Ctrl","Case"),direction="<")
    fmf37perfm <- roc(response=fh$Phenotype,predictor=fh$FMFSCORE37,ci=TRUE,levels=c("Ctrl","Case"),direction=">")
    predt <- data.frame()
    fh$Pheno <- ifelse(fh$Phenotype=="Ctrl",0,1)
    fh$log10FMFSCORE37 <- log10(fh$FMFSCORE37)

    trainallglm <- glm(Pheno ~ PEscore+log10FMFSCORE37+SeqFF, family=binomial(link='logit'), data=fh)
    print(summary(trainallglm))

    n_train <- nrow(fh)
    for (i in 1:n_train) {
        traindt <- fh[-i,]
	testvars <- as.data.frame(fh[i,c("PEscore","log10FMFSCORE37","SeqFF")])
	trainglm <- glm(Pheno ~ PEscore+log10FMFSCORE37+SeqFF, family=binomial(link='logit'), data=traindt)
    ...
    print(head(fh))
    outdt <- cbind(fh[,c("SubjectID","Phenotype","PEscore","PredClass","FMFSCORE37","FMFRISK37","Center","SeqFF")],predt)
    write.table(outdt, trainoutfh, row.names=FALSE, quote=FALSE, sep="\t")
...
#DiscoveryCombineSeqFF_FMF(infh,trainrocfig, trainoutfh, traincombrocfig)
...
ValidCombineSeqFF_FMF <- function(infh, validoutfh, validrocfig, valid1rocfig, valid2rocfig) {
    ...
    testvars <- testdt[,c("PEscore","log10FMFSCORE37","SeqFF")]

    testpred <- predict(trainglm,newdata=testvars,type="response")
    testpredclass <- ifelse(testpred > 0.5,"Case","Ctrl")
    predt <- cbind(testdt[,c("SubjectID","Phenotype","PEscore","PredClass","FMFSCORE37","log10FMFSCORE37","FMFRISK37","Center","SeqFF")],testpred,testpredclass)
    write.table(predt,validoutfh,row.names=FALSE,quote=FALSE,sep="\t")

    validpespf <- roc(response=predt$Phenotype,predictor=predt$PEscore,ci=TRUE,levels=c("Ctrl","Case"),direction="<")
    validlfmf37pf <- roc(response=predt$Phenotype,predictor=predt$log10FMFSCORE37,ci=TRUE,levels=c("Ctrl","Case"),direction=">")
    validpeadSeqFFlfmf37pf <- roc(response=predt$Phenotype,predictor=predt$testpred,ci=TRUE,levels=c("Ctrl","Case"),direction="<")
    ...
    ZOLpredt <- subset(predt,Center=="ZOL")
    ZOLvalidpespf <- roc(response=ZOLpredt$Phenotype,predictor=ZOLpredt$PEscore,ci=TRUE,levels=c("Ctrl","Case"),direction="<")
    ZOLvalidlfmf37pf <- roc(response=ZOLpredt$Phenotype,predictor=ZOLpredt$log10FMFSCORE37,ci=TRUE,levels=c("Ctrl","Case"),direction=">")
    ZOLvalidpeadSeqFFlfmf37pf <- roc(response=ZOLpredt$Phenotype,predictor=ZOLpredt$testpred,ci=TRUE,levels=c("Ctrl","Case"),direction="<")
    pdf(valid1rocfig, height=8, width=8)
    ...
    SJBpredt <- subset(predt,Center=="SJB")
    SJBvalidpespf <- roc(response=SJBpredt$Phenotype,predictor=SJBpredt$PEscore,ci=TRUE,levels=c("Ctrl","Case"),direction="<")
    SJBvalidlfmf37pf <- roc(response=SJBpredt$Phenotype,predictor=SJBpredt$log10FMFSCORE37,ci=TRUE,levels=c("Ctrl","Case"),direction=">")
    SJBvalidpeadSeqFFlfmf37pf <- roc(response=SJBpredt$Phenotype,predictor=SJBpredt$testpred,ci=TRUE,levels=c("Ctrl","Case"),direction="<")
    pdf(valid2rocfig, height=8, width=8)
...
obsolete_CombineFMF <- function(infh,allrocoutfig, validrocoutfig,modeloutfh,predoutprefix,classoutfig) {
    fh <- fread(infh, header=TRUE, sep="\t", data.table=FALSE)
    pescoreperfm <- roc(response=fh$Phenotype,predictor=fh$PEscore,ci=TRUE,levels=c("Ctrl","Case"),direction="<")
    fmf34perfm <- roc(response=fh$Phenotype,predictor=fh$FMFSCORE34,ci=TRUE,levels=c("Ctrl","Case"),direction=">")
    fmf37perfm <- roc(response=fh$Phenotype,predictor=fh$FMFSCORE37,ci=TRUE,levels=c("Ctrl","Case"),direction=">")
    ...
    traindt <- subset(fh, Set=="Discovery")
    testdt <- subset(fh, Set=="Validation")
    trainglm1 <- glm(Pheno ~ PEscore+FMFSCORE34, family=binomial(link='logit'), data=traindt)
    sink(modeloutfh)
    print(summary(trainglm1))
    print("odds ratio")
    print(exp(coef(trainglm1)))
    
    trainglm2 <- glm(Pheno ~ PEscore+log10FMFSCORE34, family=binomial(link='logit'), data=traindt)
    print(summary(trainglm2))
    print("odds ratio")
    print(exp(coef(trainglm2)))

    trainglm3 <- glm(Pheno ~ PEscore+FMFSCORE37, family=binomial(link='logit'), data=traindt)
    print(summary(trainglm3))
    print("odds ratio")
    print(exp(coef(trainglm3)))

    trainglm4 <- glm(Pheno ~ PEscore+log10FMFSCORE37, family=binomial(link='logit'), data=traindt)
    print(summary(trainglm4))
    print("odds ratio")
    print(exp(coef(trainglm4)))
    sink()

    testvars1 <- testdt[,c("PEscore","FMFSCORE34")]
    testvars2 <- testdt[,c("PEscore","log10FMFSCORE34")]
    testvars3 <- testdt[,c("PEscore","FMFSCORE37")]
    testvars4 <- testdt[,c("PEscore","log10FMFSCORE37")]

    testpred1 <- predict(trainglm1,newdata=testvars1,type="response")
    testpredclass1 <- ifelse(testpred1 > 0.5,"Case","Ctrl")
    predt1 <- cbind(testdt[,c("SubjectID","Phenotype","PEscore","PredClass","FMFSCORE34","FMFRISK34","FMFSCORE37","FMFRISK37")],testpred1,testpredclass1)
    ...
    predt2 <- cbind(testdt[,c("SubjectID","Phenotype","PEscore","PredClass","FMFSCORE34","FMFRISK34","FMFSCORE37","FMFRISK37")],testpred2,testpredclass2)
    ...
    predt3 <- cbind(testdt[,c("SubjectID","Phenotype","PEscore","PredClass","FMFSCORE34","FMFRISK34","FMFSCORE37","FMFRISK37")],testpred3,testpredclass3)
    ...
    predt4 <- cbind(testdt[,c("SubjectID","Phenotype","PEscore","PredClass","FMFSCORE34","FMFRISK34","FMFSCORE37","FMFRISK37")],testpred4,testpredclass4)
    ...
    validpespf <- roc(response=testdt$Phenotype,predictor=testdt$PEscore,ci=TRUE,levels=c("Ctrl","Case"),direction="<")
    validlfmf34pf <- roc(response=testdt$Phenotype,predictor=testdt$log10FMFSCORE34,ci=TRUE,levels=c("Ctrl","Case"),direction=">")
    validlfmf37pf <- roc(response=testdt$Phenotype,predictor=testdt$log10FMFSCORE37,ci=TRUE,levels=c("Ctrl","Case"),direction=">")
    validpeadlfmf34pf <- roc(response=predt2$Phenotype,predictor=predt2$testpred2,ci=TRUE,levels=c("Ctrl","Case"),direction="<")
    validpeadlfmf37pf <- roc(response=predt4$Phenotype,predictor=predt4$testpred4,ci=TRUE,levels=c("Ctrl","Case"),direction="<")
    ...
    classdt <- merge(predt2,predt4,by=c("SubjectID","Phenotype","PEscore","PredClass","FMFSCORE34","FMFRISK34","FMFSCORE37","FMFRISK37"))
    print(head(classdt))
    classdf <- classdt[,c("SubjectID","Phenotype","PredClass","FMFRISK34","FMFRISK37","testpredclass2","testpredclass4")]
