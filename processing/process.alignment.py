...
def write_sub_script_DNAalign(DNASampleIDs, fastqdir, scriptdir, workdir, logdir):
    ...
    for sampleid in DNASampleIDs:
        ...
        with open(samplescript, 'w') as fo:
            ...
            #directional library
            DNAalign_task = '%s -N 1 -L 20 --multicore 6 --dovetail --gzip --samtools_path %s --genome %s -1 %s -2 %s --output_dir %s --temp_dir %s\n\n' % (bismark, samtoolsdir, bismarkreference, trimFQR1, trimFQR2, sample_sub_workdir, sample_sub_workdir)
...
def write_sub_script_coverage(DNASampleIDs, scriptdir, workdir, logdir):
    ...
    for sampleid in DNASampleIDs:
        ...
        with open(samplescript, 'w+') as fo:
            ...
            ontarbam = '%s/%s.merge.deduplicated.sorted.ontar.bam' % (DNAalign_sub_workdir, sampleid)
            targetbam_task = '%s view -hbS %s -L %s > %s\n\n' % (samtools, infilteredbam, target, ontarbam)
            fo.write(targetbam_task)
            allinsert = '%s.merge.deduplicated.sorted.insert_size_metrics' % (sampleid)
            ...
            ontarinsert = '%s.merge.deduplicated.sorted.ontar.insert_size_metrics' % (sampleid)
